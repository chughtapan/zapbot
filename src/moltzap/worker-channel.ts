...
//
// Workers now require pre-minted credentials from the bridge:
//   - `MOLTZAP_AGENT_KEY` (or legacy `MOLTZAP_API_KEY`)
//   - `MOLTZAP_LOCAL_SENDER_ID`
//
// The transitional self-register path has been removed (sbd#205).
...
export type WorkerCredentialsResolveError =
  | { readonly _tag: "WorkerCredentialsMissingAgentKey"; readonly reason: string }
  | { readonly _tag: "WorkerCredentialsMissingSenderId"; readonly reason: string };

/**
 * Resolve worker credentials from the spawn env. Requires both
 * `MOLTZAP_AGENT_KEY` (or legacy `MOLTZAP_API_KEY`) and
 * `MOLTZAP_LOCAL_SENDER_ID` to be set. These are pre-minted by the bridge
 * before the worker process is spawned (architect rev 4 §2).
 *
 * This is a fail-fast enforcement of the no-self-register invariant from
 * zapbot#336 path b. Workers are channel-plugin peers, not MoltZapApp
 * consumers, and must never call `apps/register` or `apps/create`.
 */
export async function resolveWorkerCredentials(
  ...
  const senderId = trim(env.MOLTZAP_LOCAL_SENDER_ID);
  if (senderId === null) {
    return err({
...
export function formatWorkerCredentialsError(error: WorkerCredentialsResolveError): string {
  switch (error._tag) {
    case "WorkerCredentialsMissingAgentKey":
      return error.reason;
    case "WorkerCredentialsMissingSenderId":
      return error.reason;
    default: {
